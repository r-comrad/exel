...
void foo(std::vector<int> st, std::vector<int> en, funkI aCond)
{
...
void A(std::vector<int> dir, void lambda(std::vector<int> st, std::vector<int> en, funkI), funkI aCond)
{
	std::string s;
...
void B(checkD aComp, double aVal, checkD aResComp)
{
...
int main()
{
	...
	//A({ -1, 1 }, foo, [](int a, int b) {return std::max(a, b); });
